...
class Camera {
  constructor({ target = [0, 0, 0], up = [0, 1, 0], camera = [], defaultCameraMode, min_phi = 1e-6, max_phi = Math.PI - 1e-6, min_radius = 1 } = {}) {
    ...
    gl.canvas.addEventListener("mousemove", this.handleMouseMove.bind(this));
    gl.canvas.addEventListener("touchstart", this.handleTouchStart.bind(this));
    gl.canvas.addEventListener("touchmove", this.handleTouchMove.bind(this));
    gl.canvas.addEventListener("wheel", this.handleWheel.bind(this));
    document.addEventListener("keydown", this.handleKeyDown.bind(this));
    document.addEventListener("keyup", this.handleKeyUp.bind(this));
    gl.canvas.addEventListener("mouseup", this.handleMouseUp.bind(this));
  ...
  handleMouseMove(e) {
    ...
    this.phi = this.phi + e.movementY * 0.01 * 0.5;
  ...
  handleTouchMove(e) {
    ...
    this.lastTouch.clientX = touch.clientX;
    this.lastTouch.clientY = touch.clientY;
  ...
  handleKeyDown(e) {
    if (!this.freeFly || this.disableMovement || this.keyStates[e.code] == null) return;
  ...
  getPos(radius = this.radius) {
    const pos = [radius * Math.sin(this.phi) * Math.cos(this.theta), radius * Math.cos(this.phi), radius * Math.sin(this.phi) * Math.sin(this.theta)];
  ...
  update(is_dynamic = false) {
    ...
    const dot = this.lastViewProjMatrix[2] * this.vpm[2] + this.lastViewProjMatrix[6] * this.vpm[6] + this.lastViewProjMatrix[10] * this.vpm[10];
