...
function sortGaussiansByDepth(depthIndex, gaussians, viewMatrix, sortingAlgorithm) {
  const calcDepth = (i) => {
    const pos = i * 3;
    return gaussians.positions[pos] * viewMatrix[2] + gaussians.positions[pos + 1] * viewMatrix[6] + gaussians.positions[pos + 2] * viewMatrix[10];
  };

  switch (sortingAlgorithm) {
    case "Array.sort": {
      const indices = Array.from({ length: gaussians.count }, (_, i) => ({ depth: calcDepth(i), index: i }))
