...
onmessage = function(event) {
    ...
    // Sort gaussians event
        ...
        Array.from({ length: gaussians.count }).forEach((_, j) => {
            const i = depthIndex[j];
            let colors;

            if (n_sh_coeffs > 3) {
                // console.log("if");
                const campos = { x: event.data.campos[0], y: event.data.campos[1], z: event.data.campos[2] };
            ...
            // Update data.colors based on colors
            data.colors[j * 3] = colors.x;
            data.colors[j * 3 + 1] = colors.y;
            data.colors[j * 3 + 2] = colors.z;

            // Update data.positions based on gaussians.positions
            ...
            if (isAvatar) {
                ...
                // Define cov3Ds for avatar
                cov3Ds = new Float32Array([scale ** 2, 0, 0, scale ** 2, 0, scale ** 2]);
            } else {
                // Define cov3Ds based on gaussians.cov3Ds
                data.opacities[j] = gaussians.opacities[i];
                cov3Ds = gaussians.cov3Ds.slice(i * 6, i * 6 + 6);
            }

            // Update data.cov3Da and data.cov3Db based on cov3Ds
            data.cov3Da[j * 3] = cov3Ds[0];
            data.cov3Da[j * 3 + 1] = cov3Ds[1];
            data.cov3Da[j * 3 + 2] = cov3Ds[2];
            data.cov3Db[j * 3] = cov3Ds[3];
            data.cov3Db[j * 3 + 1] = cov3Ds[4];
            data.cov3Db[j * 3 + 2] = cov3Ds[5];
        });
...
function sortGaussiansByDepth(depthIndex, gaussians, viewMatrix, sortingAlgorithm) {
    ...
    // https://github.com/antimatter15/splat
        ...
        for (let i = 0; i < gaussians.count; i++) {
            ...
            // sizeList[i] = depth
            sizeList[i] = (calcDepth(i) * 4096) | 0;
            maxDepth = Math.max(maxDepth, sizeList[i]);
            minDepth = Math.min(minDepth, sizeList[i]);
        }
